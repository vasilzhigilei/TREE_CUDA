...
#NVCC := $(CUDA_PATH)/bin/nvcc -ccbin $(GCC) # use if linking to CUDA_PATH
NVCC := nvcc -ccbin $(GCC) # use if using the command "module load cuda-toolkit-#"
...
GENCODE_FLAGS   ?= $(GENCODE_SM30) $(GENCODE_SM32) $(GENCODE_SM35) $(GENCODE_SM37) $(GENCODE_SM50) $(GENCODE_SMXX)
