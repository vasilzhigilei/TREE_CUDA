...
Eqclass::~Eqclass(){
	/*for(map<int, list<vector<int> > >::iterator it = embeddingList.begin(); it != embeddingList.end(); it++){
		list<vector<int> > lst = it->second;
		for(list<vector<int> >::iterator it_lst = lst.begin(); it_lst != lst.end(); it_lst++) {
			delete (*it_lst);
		}
	}*/
}
